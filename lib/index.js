...
const createListenHook = stack => (rules, source, target) => (location, action) => {
...
export const link = (rules, a, b) => {
    ...
    a.history.listen(
        createListenHook(listenCallbacks[a.key])(rules, a, b)
    );

    b.history.listen(
        createListenHook(listenCallbacks[b.key])(rules, b, a)
    );
